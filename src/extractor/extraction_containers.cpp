...
void ExtractionContainers::writeNamesPB(const std::string &name,
                                        const std::vector<unsigned> name_offsets,
                                        const std::vector<unsigned char> &name_char_data)
{
    pbmldnames::Name pb_names;
    for (auto index : util::irange<std::size_t>(0, name_offsets.size()))
    {
        pb_names.add_offset(name_offsets[index]);
    }
    ...
    pb_names.SerializeToOstream(&pb_output);
}
...
void ExtractionContainers::WriteCharData(const std::string &file_name)
{
    ...
    writeNamesPB(file_name, name_offsets, name_char_data);
